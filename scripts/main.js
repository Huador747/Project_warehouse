...
function fillForm(product) {
  ...
  document.querySelector(".search-product-input") &&
...
function renderProductsTable(products, page = 1) {
  ...
  if (!pageProducts || pageProducts.length === 0) {
    productList.innerHTML =
      '<tr><td colspan="10" class="no-data">ไม่พบข้อมูลสินค้า</td></tr>';
    return;
  }
...
function loadProducts() {
  fetch(`${BACKEND_URL}/products`)
    ...
    .catch((error) => {
      ...
      const productList = document.getElementById("product-list");
      productList.innerHTML =
        '<tr><td colspan="10" class="error">เกิดข้อผิดพลาดในการโหลดข้อมูล</td></tr>';
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Navigation
  document
    .querySelector(".function-btn-add")
    ?.addEventListener("click", (e) => {
      e.preventDefault();
      window.location.href = "addproduct.html";
    });
  document
    .querySelector(".function-btn-edit")
    ?.addEventListener("click", (e) => {
      e.preventDefault();
      window.location.href = "editproduct.html";
    });
  document
    .querySelector(".function-btn-delete")
    ?.addEventListener("click", (e) => {
      e.preventDefault();
